...
def iqblk_plot(iqSR, recLen, IQ, show=True, save=False, path=".", filename="IQ_vs_Time_Plot"):
    # Plots IQ data
    # Input: Complex numpy array of (I + Qj) values
    time = np.linspace(0, recLen / iqSR, recLen)
...
def iqblk_master(cf=1e9, refLevel=0, iqBw=40e6, recordLength=1024, save=False,
    filename="IQ_Data", path='.', plot=False, fftPlot=False, numFFTs=2, savefig=False, figname="IQ_vs_Time_Plot"):
    # Inputs:
    ...
    # Set parameters
    config_iqblk(cf, refLevel, iqBw, compRecLen)
    ...
    if plot:
        ...
        time = np.linspace(0, recordLength / iqSR, recordLength)
        iqblk_plot(time, iqArr, save=savefig, path=path, filename=figname)

    if fftPlot:
        iqFFT_m4s = fft_m4s(iqSlice)
        freqData = generate_spectrum(cf, iqSR, recordLength)
        calcBw = freqData[-1] - freqData[0]
        fft_plot(freqData, iqFFT_m4s[2], title="Mean FFT")

    # Save data to file
    if save:
        headerInfo = ("IQ DATA FILE\nCF: " + str(cf) + " Hz, Ref. Level: "
            + str(refLevel) + " dBm, BW: " + str(iqBw) + " Hz, Rec. Length: " 
            + str(recordLength) + " samples, SR: " + str(iqSR))
...
# Take IQ Data FFT + Calculate Mean
def fft_mean(iqData, power=True):
    # iqData: IQ data array
...
def wifi_fft(cf=2.437e9, refLevel=-40, iqBw=40e6, recordLength=1024):
    ...
    config_iqblk(cf, refLevel, iqBw, compRecLen)
    iqSampleRate = IQBLK_GetIQSampleRate()
...
def sr_test_fft(cf=2.437e9, refLevel=-60, recordLength=1024):
    ...
    for (i, bw) in enumerate(bwArray):
        ...
        srArray[i] = IQBLK_GetIQSampleRate()
        iqArray = np.zeros((numFFTs, recordLength), dtype=complex)
        for (num, vals) in enumerate(iqArray):
            iqArray[num] = iqblk_collect(recordLength)
        m4s_temp = fft_m4s(iqArray)
...
def gain_char(minRL, maxRL, numRLs, noiseFig=False, trunc=False, trunc1=False, fftPlot=False, hist=False,
    ...
    numSets = 1000 # Number of IQ blocks
    ...
    IQBLK_SetIQRecordLength(compRecLen)

    # Generate FFT frequencies
    sampRate = IQBLK_GetIQSampleRate()
    ...
    for (i, RL) in enumerate(refLev):
        ...
        FFTs[i] = fft_mean(iqSlice, power=True) # Store mean FFT
...
def gainChar_ENBW(freqs, FFTs, refLevels):
    ...
    for (i, RL) in enumerate(refLevels):
        linFFT = 10**((FFTs[i]-30)/20) # Convert dBm to psuedo-volts
...
def transferFunc(cf=1e9, refLev=-40, bw=40e6):
    ...
    DEVICE_Connect()
    config_iqblk(cf, refLev, bw, compRecLen)
    iqSampleRate = IQBLK_GetIQSampleRate()
...
#iqstream_test()
# iqblk_master(refLevel=-70, fftPlot=True, numFFTs=1000)